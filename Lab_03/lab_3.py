...
def interpolate(x, y, x_value):
    ...
    for i in range(2, n):
        si[i + 1] = D[i] / (B[i] - A[i] * si[i])
        eta[i + 1] = (A[i] * eta[i] + F[i]) / (B[i] - A[i] * si[i])

    c = [0 for i in range(n + 1)]
    for i in range(n - 2, -1, -1):
        c[i] = si[i + 1] * c[i + 1] + eta[i + 1]
...
x_step = float(input("Input step for x: "))
x_amount = int(input("Input number of x: "))
